...
public class NetworkDebugStart : MonoBehaviour {
	...
	void OnGUI(){
		...
		roomName = GUI.TextField(new Rect(0, Screen.height - Screen.height/4, Screen.width, Screen.height/8), roomName, customInputField);
		
		//Host game Button
		if (GUI.Button(new Rect(0, Screen.height - Screen.height/8, Screen.width/2, Screen.height/8), "Host Game", customButton)){
			CreateRoom();
		}

		//Join game Button
		if (GUI.Button(new Rect(Screen.width/2, Screen.height - Screen.height/8, Screen.width/2, Screen.height/8), "Join Game", customButton)){
			JoinRoom();
		}		
	}
